...
# 生成图片保存的路径
CKimg_dir = '/public/data/coco/coco2017_top100_voc/JPEGImages'
# 生成标注文件保存的路径
CKanno_dir = '/public/data/coco/coco2017_top100_voc/Annotations'

CKtest_dir = '/public/data/coco/coco2017_top100_voc/ImageSets/Main'
...
def showbycv(coco, dataType, img, classes, origin_image_dir, verbose=False):
    ...
    objs = []
    with open(CKtest_dir+'test.txt', 'a+') as file:
        file.write(filename.split('.')[0]+'\n')
    for ann in anns:
        name = classes[ann['category_id']]
        if not (name in classes_list):
            continue
        if 'bbox' in ann:
            bbox = ann['bbox']
            xmin = (int)(bbox[0])
...
def main():
    base_dir = '/public/data/coco/coco2017_top100_voc'  # step1 这里是一个新的文件夹，存放转换后的图片和标注
    image_dir = os.path.join(base_dir, 'JPEGImages')  # 在上述文件夹中生成images，annotations两个子文件夹
    anno_dir = os.path.join(base_dir, 'Annotations')
    mkr(image_dir)
    mkr(anno_dir)
    origin_image_dir = '/public/data/coco/coco2017_top100'  # step 2原始的coco的图像存放位置
    origin_anno_dir = '/public/data/coco/coco2017_top100/annotations'  # step 3 原始的coco的标注存放位置
