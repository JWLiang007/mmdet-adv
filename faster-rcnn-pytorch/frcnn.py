...
class FRCNN(object):
    _defaults = {
        "model_path"    : '/workspace/mmdetection/faster-rcnn-pytorch/voc/voc_weights_resnet.pth',
        "classes_path"  : '/workspace/mmdetection/faster-rcnn-pytorch/model_data/voc_classes.txt',
        "confidence"    : 0.3,
        "iou"           : 0.3,
        "backbone"      : "resnet50",
        "cuda"          : True,
    ...
    def generate(self):
        ...
        optimizer = optim.Adam( self.model.parameters(), 1e-5, weight_decay=5e-4)
        self.train_util = FasterRCNNTrainer(self.model, optimizer)
        if self.cuda:
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image,return_det = False):
        #-------------------------------------#
        #   转换成RGB图片，可以用于灰度图预测。
        #-------------------------------------#
        ...
        with torch.no_grad():
            ...
            # losses = self.train_util.train_step(images, roi_cls_locs, roi_scores, 1)
            # print(losses)
            ...
            conf = outputs[:, 5]
            if return_det:
                det_bbox = deepcopy(bbox)
                det_label = deepcopy(label)
                det_conf = deepcopy(conf)
                det_images = deepcopy(images)
