...
class PRFA(Attack):
    ...
    def forward(self,data):
        ...
        self.ub,self.lb = torch.max(images.view(3,-1),dim=1).values,torch.min(images.view(3,-1),dim=1).values
        self._eps = self.eps * torch.max(self.ub - self.lb )
        adv_images = images.clone().detach()
        new_data = {}
        new_data['img_metas'] = data['img_metas']
        new_data['img'] = adv_images
        adv_images = self.perturb(new_data ,data)
        data['img'][0].data[0] = adv_images
        return adv_images
    
    def losses(self,img,data):
        with torch.no_grad():
            new_data  = {}
            new_data['img'] = img
    ...
    def attack_single_run(self, new_data,data):
        ...
        with torch.no_grad():
            ...
            _data = {}
            _data['img_metas'] = data['img_metas']
            _data['img'] = deepcopy(data['img'])
            if self.norm == 'Linf':
                ...
                for i_iter in range(self.n_queries):
                    ...
                    if not self.flip_flag:
                        ...
                        # attack_points = []
                        # for bbox in bboxes:
                        #     for temp_w in range(int(bbox[0].item()), int(bbox[2].item()), 1):
                        #         for temp_h in range(int(bbox[1].item()), int(bbox[3].item()), 1):
                        ...
                        # else:
                        bbox = random.choice(_new_data['gt_bboxes'][0].data[0][0])
                        vh = random.choice(range(int(bbox[1].item()), int(bbox[3].item())))
                        vw = random.choice(range(int(bbox[0].item()), int(bbox[2].item())))
                        vh = h -s if vh + s > h else vh
                        vw = w -s if vw + s > w else vw

                        new_deltas = torch.zeros([c, h, w]).to(self.device)
                        new_deltas[:, vh:vh + s, vw:vw + s
                            ] = 2. * self._eps * self.random_choice([c, 1, 1])
                        self.flip_deltas = new_deltas.clone()
                        self.flip_flag = True
                        self.flip_zone = 'h1'
                    else:
                        flip_size = int(s/2)
                        ...
                        if self.flip_zone == 'h1':
                            flip_deltas_sign = torch.ones(( c, s, s)).to(self.device)
                            flip_deltas_sign[ :, flip_size:, :] = flip_deltas_sign[ :, flip_size:, :] * -1.0
                            new_deltas[ :, vh:vh+s, vw:vw+s] = self.flip_deltas[ :, vh:vh+s, vw:vw+s] * flip_deltas_sign
                            self.flip_zone = 'h2'
                    ...
                    x_new = torch.min(torch.max(x_new, x_curr - self._eps),
                        x_curr + self._eps)
                    ...
                    if i_iter % (self.n_queries//5) == 0:
                        print('iter-'+str(i_iter)+ ' loss :',loss_min)
    ...
    def perturb(self, new_data, data):
        """
